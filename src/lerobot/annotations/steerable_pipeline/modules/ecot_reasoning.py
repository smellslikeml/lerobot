...
The prompt is inlined as a module constant rather than living in
``prompts/*.txt`` so this module stays self-contained.
"""
...
_PROMPT_TEMPLATE = (
    "You are generating one structured Embodied Chain-of-Thought (ECoT) "
    "reasoning trace for a single segment of a robot manipulation episode.\n\n"
    'Task: "{episode_task}".\n'
    "This segment is centered at {segment_time:.2f}s into the episode.\n\n"
    "Produce the four reasoning stages that ZR-0 shows generalize across "
    "robot embodiments:\n"
    ...
    "  4. subtask_decomposition — the immediate atomic action for THIS segment.\n\n"
    "Output strictly valid JSON only, no prose, exactly this shape:\n"
    "{{\n"
...
class EcotReasoningModule:
    ...
    def run_episode(self, record: EpisodeRecord, staging: EpisodeStaging) -> None:
        ...
        for (ts, _messages), result in zip(per_call, results, strict=True):
            ...
            rows.append(
                {
                    "role": "assistant",
                    "content": json.dumps(reasoning, sort_keys=True),
                    "style": "ecot",
    ...
    def _build_messages(self, record: EpisodeRecord, center_ts: float) -> list[dict[str, Any]]:
        ...
        prompt = _CONTACT_SHEET_NOTE + _PROMPT_TEMPLATE.format(
